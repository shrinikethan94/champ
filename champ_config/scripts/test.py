...
class Follower:
	...
	def image_callback(self, msg):
		...
		if (self.trackMethod == 'contours'):
			...
			if My['m00'] > 0:
				xy = int(My['m10']/My['m00'])
				yy = int(My['m01']/My['m00'])
				cv2.circle(image, (xy, yy), 15, (100, 255, 100), 2)
				
				
			# Move the robot.
			# NOTE:  This method uses the AREA of the circle.
			self.moveMe(xr, w, self.goal_area, area, xy)
	...
	def moveMe(self, xr, w, goal, observed, xy):
		...
		if (xr == None) and self.turn == 0: 
			...
			# When close enough, stops and rotates until ball is lined up with goal	
				...
				if (xr is not None and xy is not None) or self.turn > 1:
					if self.turn == 0:
						# checking if center of ball and center of goal are lined up
						if abs(xr-xy) < 15:
						# if xr < xy1 and xr > xy2:
